...
** 
*****************************************************************************/
void UART_Handler( LPC_USART_TypeDef *UARTx, int ch )
{
...
/**
 * UART送信バッファに、1データを書き込む。
 */
void UART_SetTxData(int ch, int data) {
	int count = UART_GetTxCount(ch);
	if ( count < BUFSIZE ) {
		uartContext[ch].txBuffer[uartContext[ch].txWritePointer++] = data;
		if ( uartContext[ch].txWritePointer >= BUFSIZE ) {
			uartContext[ch].txWritePointer = 0;
		}
		if ( ch == 0 ) {
			LPC_USART0->INTENSET = TXRDY;
		} else {
			LPC_USART1->INTENSET = TXRDY;
		}
	}
}

/**
 * UART送信バッファから、1データを取り出す。
 */
int UART_GetTxData(int ch) {
	int data = -1;
	int count = UART_GetTxCount(ch);
	if ( count > 0 ){
		data = uartContext[ch].txBuffer[uartContext[ch].txReadPointer++];
		if ( uartContext[ch].txReadPointer >= BUFSIZE ) {
			uartContext[ch].txReadPointer = 0;
		}
	}
	return data;
}
...
/**
 * UART送信バッファに書き込まれたサイズを返す。
 */
int UART_GetTxCount(int ch){
	int count = uartContext[ch].txWritePointer - uartContext[ch].txReadPointer;
	if ( count < 0 ) {
		count += BUFSIZE;
	}
	return count;
...
/**
 * UART受信バッファに、1データを書き込む。
 */
void UART_SetRxData(int ch, int data) {
	int count = UART_GetRxCount(ch);
	if ( count < BUFSIZE ) {
		uartContext[ch].rxBuffer[uartContext[ch].rxWritePointer++] = data;
		if ( uartContext[ch].rxWritePointer >= BUFSIZE ) {
			uartContext[ch].rxWritePointer = 0;
		}
	}
}

/**
 * UART受信バッファから、1データを取り出す。
 */
int UART_GetRxData(int ch){
	int data = -1;
	int count = UART_GetRxCount(ch);
	if ( count > 0 ) {
		data = uartContext[ch].rxBuffer[uartContext[ch].rxReadPointer++];
		if ( uartContext[ch].rxReadPointer >= BUFSIZE ) {
			uartContext[ch].rxReadPointer = 0;
		}
	}
	return data;
}
...
/**
 * UART受信バッファに書き込まれたサイズを返す。
 */
int UART_GetRxCount(int ch){
	int count = uartContext[ch].rxWritePointer - uartContext[ch].rxReadPointer;
	if ( count < 0 ) {
		count += BUFSIZE;
	}
	return count;
